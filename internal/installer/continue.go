...
// ContinueInstaller handles installation for Continue
type ContinueInstaller struct {
	BaseInstaller
}

// NewContinueInstaller creates a new Continue installer
func NewContinueInstaller() *ContinueInstaller {
	return &ContinueInstaller{
		BaseInstaller: BaseInstaller{
			Config: config.GetToolConfig(config.ToolContinue),
		},
	}
}

// SupportsType returns true if Continue supports the given resource type
func (c *ContinueInstaller) SupportsType(t resource.Type) bool {
	switch t {
...
// Install installs a resource to Continue's config directory
func (c *ContinueInstaller) Install(res *resource.Resource, global bool, force bool) ([]string, error) {
	if !c.SupportsType(res.Type) {
		return nil, fmt.Errorf("continue does not support resource type: %s", res.Type)
	}
...
// Uninstall removes a resource from Continue's config directory
func (c *ContinueInstaller) Uninstall(res *resource.Resource, files []string, global bool) error {
...
func (c *ContinueInstaller) InstallFromReader(res *resource.Resource, content io.Reader, global bool, force bool) (string, error) {
	if !c.SupportsType(res.Type) {
		return "", fmt.Errorf("continue does not support resource type: %s", res.Type)
	}
